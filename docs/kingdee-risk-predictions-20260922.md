...
| P1-6 | GL_FAILED 流水在凭证中心永久无法重推 | 已实施 | CAS 认领补 GL_FAILED + GL_PUSHED 幂等早返回 |
...
## P1-6 · GL_FAILED 流水在凭证中心永久无法重推（✅ 已实施，端到端测试抓出）

### 怎么发现的
2026-09-22 真账套端到端测试（app 推送流水 18）返回
`409 Statement push is already in progress or has completed`——但流水 18 实际是
`GL_FAILED`（失败待重试），与报错语义完全不符。

### 机制（代码实证）
`StatementService.pushVoucher` 的 CAS 认领条件是
`push_status IN ('NOT_PUSHED','FAILED')`——**漏了 `GL_FAILED`**。而总账落点失败写的正是
`GL_FAILED`（区别于出纳单落点的 `FAILED`）。后果：
- 总账推送失败的流水**永久无法通过凭证中心重推**（唯一出路是手工改库或撤回重制）；
- 报错文案误导（说「已在推送中或已完成」，用户无从判断下一步）。

这正是 W15「根因修了、重推还是不行」的**另一半原因**——即使账簿修复上线，
失败流水也推不出去。批量推送路径（`batchPush`）同样受影响：`GL_PUSHED` 会被误报为
FAILED 行、`GL_PUSHED` 不参与幂等跳过。

### 修复（本批）
1. CAS 认领条件补 `GL_FAILED`；
2. 幂等早返回改用 `PUSH_COMPLETED`（{PUSHED, GL_PUSHED}）——原先只认 `PUSHED`，
   `GL_PUSHED` 会落到 CAS 被拒成误导性 409；
3. 批量推送的「已推送跳过」与「成功行判定」同步改用 `PUSH_COMPLETED`。

### 验证（2026-09-22 真账套，端到端）
`StatementServiceGlPushIntegrationTest` +2 例（GL_FAILED 可重推 / GL_PUSHED 幂等不重复推送）；
真账套：流水 17（上海图虫）复核通过 → 凭证中心推送 → **`GL_PUSHED` + 金蝶凭证号 16113**，
app 日志 `acctbook=410 org=410`、`#2 1002 FDETAILID__FF100002=110922659010201`，
金蝶侧只读回查 `GL_VOUCHER 16113`（账簿 410 / FDocumentStatus=A 暂存草稿）真实存在。

---

## 端到端验证结论（2026-09-22，真账套）

| 层 | 方法 | 结果 |
|---|---|---|
| 报文级 | 无维度科目 + 银行维度分录，账簿410+组织410+图虫档案 | ✅ 成功（单号 16112，即删） |
| 报文级 | 同报文改回修复前组合（账簿400+组织410） | ❌ 复现「必录维度未录入或不可用：银行账号」 |
| 部署级 | 线上 app.jar 字节码含 4 项修复 | ✅ 全部命中 |
| 端到端 | 凭证中心推送流水 17 → 金蝶 | ✅ GL_PUSHED + 凭证 16113（账簿 410，草稿态） |
| 端到端 | 推送流水 18（GL_FAILED） | ❌ 409 → **抓出 P1-6**，已修复 |
