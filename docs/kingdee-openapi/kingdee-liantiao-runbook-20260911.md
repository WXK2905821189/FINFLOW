...
| 我方银行账户 | 1234567 网银01 | FINFLOW 侧仍为测试账户，无真实联通 | ✅ **账户级映射已落地（2026-09-21，V41）**：银行账户页「金蝶账户映射」抽屉一键自动匹配（按账号 + 公司→组织消歧，唯一命中自动写回，多义/虚拟账户人工指定）；制证时按流水所属账户注入维度，**未映射即阻断并提示**。`KINGDEE_DEFAULT_BANK_ACCOUNT_NUMBER` 降级为「流水无账户归属时的兜底」 |
...
KINGDEE_DEFAULT_BANK_ACCOUNT_NUMBER=11050160520009100036  # 兜底：仅流水无账户归属时使用；正常走账户级映射（银行账户页「金蝶账户映射」）
```
