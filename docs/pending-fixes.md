...
### 前置 2：「业务线」维度决策（业务侧）—— ✅ 已决策（2026-09-21）
- 账套 `BD_FLEXITEMPROPERTY` 的 14 个维度类型里**没有「业务线」**，而图虫规则大量引用。
- **用户拍板：用「项目 `ZDY0002` / `FF100003`」承载**（与 `PROJECT` 共用槽位，值为项目档案编码）；
  V42 seed 已按 `dimension_type=BUSINESS_LINE` 写入该配置。
- 若将来金蝶侧新增独立「业务线」维度：改「核算维度配置 › 槽位配置」一行即可，**不改代码**。
- 待办遗留：业务线（项目）的**档案编码清单**仍需财务整理后批量导入值映射表。
