...
- CI 的 `Release contract` 会拒绝提交明确命名的银行或金蝶生产凭据，并要求 V1、V2、V3 三个迁移版本均已存在。该检查不能替代人工密钥审查。

## 数据库迁移

1. 在变更窗口前对 RDS 执行可恢复的全量备份，并验证能恢复到独立实例。
2. 使用受限账号连接目标生产库，确认库名、字符集、时区以及网络连通性正确。
3. 部署新的后端 JAR，并以 `SPRING_PROFILES_ACTIVE=prod` 启动。应用启动时 Flyway 必须按版本顺序执行基础迁移 `V1` -> `V2` -> `V3` -> `V4` -> `V5` -> `V6`，数据库厂商迁移 `V7`，再执行 `V8`。
4. `V7` 位于 `db/vendor-migration/{vendor}`，用于按 H2/MySQL 厂商处理遗留流水号唯一约束；`V8` 增加支付请求编号和 UNKNOWN 人工处置字段。后端已在 H2 开发库完成 V1 -> V8 空库启动验证；进入生产变更窗口前仍需在隔离 MySQL 完成空库全量、已有库增量和重复启动验证。
...
| SYS-02 迁移 | 部分通过 | H2 已验证 V1-V6、厂商 V7 和 V8；真实 MySQL 空库/增量/重复启动仍待执行。 |
