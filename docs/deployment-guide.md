...
连接与运营模块一期只管理连接档案、受控任务及操作日志，用于展示和审计模拟或文件导入流程。它不是银行接入层，也不是生产密钥存储服务：
...
4. 若连接与运营模块出现异常，停止其受控任务和页面入口，保留操作日志以供审计；确认没有真实银行或金蝶请求、凭据或定时任务被启用后，再回切流量。
...
| DEP-P1-004 | P1 | 浏览器自动化环境无法访问本机开发服务器，浏览器 E2E 与运行时敏感扫描尚无可复核证据。 | 在可访问候选服务的测试浏览器完成登录、RBAC、六类查询、DOM/缓存/控制台和错误页扫描。 |
