...
public class BankDataQueryService {
    ...
    public static final String CROSS_COMPANY_PERMISSION = BankDataTaskScope.CROSS_COMPANY_PERMISSION;

    /** 任务范围 / 血缘 / 账户标签 / 权限判定共享支撑（与导出服务共用，见 BankDataTaskScope）。 */
    ...
    public BankDataQueryService(BankDataTaskScope taskScope,
    ...
    public BankDataProjectionPageResponse<?> queryProjection(Long userId, String resource,
        ...
        List<Long> taskIds = taskScope.scopedTaskIds(companyIds, syncJobNo, requestId);
        if (!taskScope.realDirectConnected()) {
            return notConnectedPage(page, size);
        }
        List<Long> realTasks = taskScope.realTaskIds(companyIds);
    ...
    public PageResponse<BankDataBalanceResponse> listBalances(Long userId, int page, int size,
        ...
        List<Long> taskIds = taskScope.scopedTaskIds(List.of(companyId), taskNo, requestId);
